...
\textit{high end} costosos que proporcionen espai per a CPUs, controladors i memòria
...
\verb|/home| a través de NFS i es confien l'una a l'altra per executar shells remotes
...
Tot i buscar per Internet la existència d'altre software de clustering, no hem
...
representació d'una zona de memòria) i l'usuari que accedeix al fitxer, de forma
...
A més \proc en podem dir que és un pseudo-sistema de fitxers (o sistema de
...
la opció \verb|CONFIG_PROC_FS=Y|. També haurem de seleccionar la opció
\verb|CONFIG_SYSCTL=Y| per a poder modificar el valor de determinats
paràmetres.  La majoria de distribucions inclouen nuclis compilats amb aquesta
opció i en general és aconsellable seleccionar-la en el moment de recompilar el
nucli.
...
informació sobre la utilització de la memòria \ldots).
...
Al fitxer \texttt{proc.txt} (disponible al directori de documentació de
sistemes de fitxers del codi font del nucli de Linux) hi ha informació detallada
de tot el que podem trobar dins de \proc.
Un altre document d'interès és \texttt{ip-sysctl.txt}, disponible al directori
de documentació sobre treball en xarxa.
...
(desactivat) al paràmetre \verb|ICMP_ECHO_IGNORE_ALL|.
...
Tot i la conveniència del \proc de Linux per al que volem fer, al ser una
interfície reduïda del VFS --- simplifica moltes de les seves operacions i
n'elimina d'altres ---, no té una operació que, per a nosaltres, és molt
important: la de moure objectes dins l'arbre de directoris (en l'entorn del VFS
s'anomena \texttt{rename}).

Precisament per això utilitzem el VFS directament, tot i que a efectes pràctics
els canvis són pocs i només al nivell d'implementació, doncs com ja s'ha dit,
\proc ofereix una sèrie d'operacions ja simplificades per a crear arbres de
...
Emperò tampoc hem trobat res en d'altres sistemes, excepte els translators que
ja hem comentat per a GNU/Hurd, de manera que no hem aconseguit trobar res per
encabir en aquesta secció.
