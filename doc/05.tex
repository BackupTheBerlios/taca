...
tal de reduïr el tamany de la implementació (doncs fer-la completa requereix
...
Al ser un sistema de fitxers virtual, tota la inforació mostrada es genera de
...
En el següent codi (\verb"frontend/linux/*.c") generem el contingut per al directori
...
\begin{verbatim}
int tacafs_f_readdir( struct file *file, void *dirent, filldir_t filldir ) {
	[...]
...
struct dentry *tacafs_i_lookup (struct inode *parent_inode, struct dentry *dentry) {
	...
	file_inode->i_fop = &tacafs_dir_fops;	// file_operations
	file_inode->i_op = &tacafs_iops;	// inode_operations
...
En quant als fitxers, les operacions que duu associades són el \textbf{lookup},
que duu a terme la mateixa tasca que en el cas dels directoris, el \textbf{read}
i el \textbf{write} que són els encarregats de llegir i escriure els continguts
del fitxer i el \textit{rename} que és l'encarregat de tractar amb el moviment
de fitxers i directoris.

Les funcions \textbf{read} i \textbf{write} no ens ha calgut implementar-les per
l'objectiu que ens hem fixat.

La funció clau per a la migració de processos és \textbf{rename}, però no la hem
...
(fitxer \verb"backend/backend.h") per a tractar des del \textit{frontend} amb
el \textit{backend}.
...
Aquest exemple es troba a \verb"backend/dummy/dummy.c".


\subsubsection{Problema d'OpenMosix}
...
\verb"/proc/hpc", sense exportar cap símbol utilitzable des del propi nucli, de
forma que l'única manera d'accedir-hi és a través del VFS, el qual només és
accessible a través de programes amb un contexte propi (com serien programes en
mode usuari o threads de kernel).

Aquest intent d'implementació es troba a \verb"backend/openmosix/".
...
que duria a terme la petició a través de \verb"/proc/hpc/" i retornaria el
resultat altra vegada al nostre \textit{backend}.

\begin{figure}[hp]
   \centering
   \includegraphics[scale=0.4]{solucio-espai-usuari.eps}
...
documentació tant externa com a dins del propi codi és molt minsa.
